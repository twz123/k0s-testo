...
func (s *DualstackSuite) SetupSuite() {
	...
	createdTargetPod, err := kc.CoreV1().Pods("default").Create(s.Context(), &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{{Name: "nginx-worker0", Image: "docker.io/library/nginx:1.28.0-alpine"}},
	...
	sourcePod, err := kc.CoreV1().Pods("default").Create(s.Context(), &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{{Name: "alpine", Image: "docker.io/library/nginx:1.28.0-alpine"}},
			NodeSelector: map[string]string{
